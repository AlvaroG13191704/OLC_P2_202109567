...
func (g *Generator) GenPrintBoolFunc() {
	...
	// add if
	g.AddIf(g.TempBoolFunc, "0", "==", labelIfTrue)
	// generate "true"
...
// Func to compare chars
// func (g *Generator) GenComparationChar(left, right, sign string) string {
...
func (g *Generator) GenComparationStringFunc() string {
	...
	// generate temps
	temp1 := g.NewTemp()
	g.TempFirstStringCompare = temp1
	temp2 := g.NewTemp()
	g.TempSecondStringCompare = temp2
	temp3 := g.NewTemp()
	temp4 := g.NewTemp()
	temp5 := g.NewTemp()
	// code
